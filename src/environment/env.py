...
class VecGenerationEnv(SubprocVecEnv):
    ...
    def step_async(self, actions: np.ndarray) -> None:
        with torch.no_grad():
            ...
            onehot_segs = process_levels(self.generator(z)) # onehot --> level, print(str(process_levels(self.generator(z), True)[0])), process_levels(self.generator(z), True)[1].to_img()
        ...
        with torch.no_grad():
            if self.init_one:
                ...
                for i in range(len(env_ids)):
                    ...
                    # np.savetxt(f'text{i}.txt', z.cpu().numpy().reshape(1, -1))
                    # print(np.sum(z.cpu().numpy().reshape(-1, 1)))
